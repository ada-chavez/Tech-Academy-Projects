...
def load_gui(self):
    ...
    #Listbox with a scrollbar and grid them
    ...
    #buttons
    self.btn_submit = tk.Button(self.master,width=12,height=2,text='Submit', command=lambda: student_tracking_func.submitToList(self))
    self.btn_submit.grid(row=9,column=2,padx=(25,0),pady=(5,10),sticky=E)
    self.btn_delete = tk.Button(self.master,width=12,height=2,text='Delete',command=lambda: student_tracking_func.onDelete(self))
    self.btn_delete.grid(row=9,column=3,padx=(15,0),pady=(5,10),sticky=E)
